...
This design simplifies the agent interaction logic and allows for dynamic scaling controlled
at Core side.
...
Replicante Core will at some point include other agent transports.
...
Replicante Core delegates security of the network to the transport layer.

## HTTP(S) transport

The HTTP transport is the easiest to use but also the least secure.

With this transport Replicante Core acts as an HTTP client for the agents.
...
although this comes at the cost of repeated TCP connection handshakes.
...
* A Certificate Authority certificate, even a self-signed one, to generate all other certificates.
...
For now, the Replicante Core configuration is set from temporary environment variables:

```bash
# One or more CAs that signed the agent's private keys.
export REPLICORE_TMP_DEFAULT_AGENTS_CA_BUNDLE="path/to/agents/CAs/bundle.pem"

# The private key that needs to be signed by a CA known to EVERY agent.
export REPLICORE_TMP_DEFAULT_AGENTS_CLIENT_KEY="private/key/used/by/core.pem"

# Start Replicante Core
replicante
```
